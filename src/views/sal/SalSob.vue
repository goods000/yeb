	...
	<div>
		...
		<div>
			<el-table
				...
				<el-table-column
				  ...
				  align="center">
					<el-table-column
					    prop="eccumulationFundPer"
					    label="比率"
					    width="70">
					</el-table-column>
					<el-table-column
					    prop="eccumulationFundBase"
					    label="基数"
					    width="70">
					</el-table-column>
				</el-table-column>
		...
		<el-dialog
		  ...
		  <div class="">
			  ...
			  <el-input v-model="salary[title]" :placeholder="'请输入'+salaryItemName[index]+'...'" v-for="(value,title,index) in salary" :key="index"
		...
		methods:{
			...
			deleteSalary(data){
				this.$confirm('此操作将永久删除['+ data.name +']工资账套, 是否继续?', '提示', {
					...
					this.deleteRequest('/salary/sob/delete/'+ data.id).then(resp => {
						if(resp){
							this.initSalaries();
						}
					})
			...
			initSalaries(){
				this.getRequest('/salary/sob/get-All').then(resp => {
					if(resp){
						this.salaries = resp;
					}
				})
