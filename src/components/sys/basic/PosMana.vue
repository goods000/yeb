<template>
	<div class="">
		<div class="">
			...
			<el-button icon="el-icon-plus" size="small" type="primary" @click="addPositon()">添加</el-button>
		</div>
		...
		methods:{
			deleteMany(){
				this.$confirm('此操作将永久删除['+ this.multipleSelection.length +']条职位, 是否继续?', '提示', {
					...
					this.deleteRequest('/system/basic/position/deletes?'+ ids).then(resp => {
						if(resp){
							this.initPositions();
						}
					})
			...
			handleDelete(index, data) {
				this.$confirm('此操作将永久删除['+ data.name +']职位, 是否继续?', '提示', {
					...
					this.deleteRequest('/system/basic/position/delete?id='+data.id).then(resp => {
						if(resp){
							this.initPositions();
						}
					})
