<template>
	<div class="">
		<div>
			...
			<el-select size="small" v-model="jl.titleLevel" placeholder="职称等级" style="margin:0 6px;">
			    <el-option
		...
		<div style="margin-top: 10px;">
			<el-table
			  ...
			  <el-table-column
				label="操作">
				<template slot-scope="scope">
					<el-button size="small" @click="showEditView(scope.row)">编 辑</el-button>
					<el-button size="small" type="danger" @click="deleteHandler(scope.row)">删 除</el-button>
				</template>
			  </el-table-column>
			</el-table>
			<el-button size="small" style="margin-top: 10px;" type="danger" :disabled="this.multipleSelection.length == 0" @click="deleteMany()">批量删除</el-button>
		</div>
		<el-dialog
		  title="编辑职称"
		  ...
		  <table>
		  	<tr>
				...
				<td><el-input size="small" v-model="updateJl.name" placeholder="添加职称名称" prefix-icon="el-icon-plus" style="margin:0 6px;"></el-input></td>
			</tr>
			<tr>
				<td><el-tag>职称等级</el-tag></td>
				<td>
					<el-select size="small" v-model="updateJl.titleLevel" placeholder="职称等级" style="margin:0 6px;">
					    <el-option
					      v-for="item in titleLevels"
					      :key="item"
					      :label="item"
		...
		methods:{
			...
			deleteHandler(data){
				this.$confirm('此操作将永久删除['+ data.name +']职称, 是否继续?', '提示', {
					...
					this.deleteRequest('/system/basic/joblevel/delete?id='+data.id).then(resp => {
						if(resp){
							this.initJls();
						}
					})
